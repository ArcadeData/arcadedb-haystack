...
documents = [
    Document(
        content=(
            "ArcadeDB is a multi-model database supporting graphs,"
            " documents, key-value, time-series, and vectors."
        ),
    ...
    Document(
        content=(
            "HNSW (Hierarchical Navigable Small World) enables"
...
pipeline.add_component(
    "retriever", ArcadeDBEmbeddingRetriever(document_store=store, top_k=3)
)
...
for doc in result["retriever"]["documents"]:
    print(
        ...
        f"  category={doc.meta.get('category')}"
