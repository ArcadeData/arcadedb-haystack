...
class ArcadeDBEmbeddingRetriever:
    """
    Retrieves documents from ArcadeDB using vector similarity (HNSW index).
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ArcadeDBEmbeddingRetriever":
        ...
        if "document_store" in init_params:
            init_params["document_store"] = ArcadeDBDocumentStore.from_dict(
                init_params["document_store"]
            )
