...
from .converters import _from_arcadedb_to_haystack, _from_haystack_to_arcadedb
from .filters import _convert_filters

logger = logging.getLogger(__name__)

# Haystack Secret helper — works with env vars, plain strings, etc.
try:
    from haystack.utils import Secret
except ImportError:
    from haystack.utils.auth import Secret  # type: ignore[no-redef]
...
class ArcadeDBDocumentStore:
    ...
    Supports HNSW vector search, Lucene full-text search, and SQL metadata filtering.
    """
    ...
    # ------------------------------------------------------------------
    # DocumentStore protocol — 4 core methods
    # ------------------------------------------------------------------
    ...
    def _embedding_retrieval(
        ...
        *,
        ...
        Returns documents ordered by ascending distance (closest first).
        """
        self._ensure_initialized()
        ...
        # Apply optional metadata filters client-side
        # (vectorNeighbors doesn't support WHERE clauses directly)
        where = _convert_filters(filters)
        ...
        for neighbor in neighbors:
            ...
            # Convert distance to a similarity score (1 - distance for cosine)
            score = 1.0 - distance
            ...
            doc = Document(
                ...
                score=score,
                # embedding is serialized as Java array ref in neighbor results,
                # skip it here — caller rarely needs it for retrieval results
            )
            documents.append(doc)

        # If filters are specified, do a post-filter on the returned docs
        if where and filters:
            filtered_ids = {
                r["id"]
                for r in self._command(
                    f"SELECT id FROM `{self._type_name}` WHERE {where}"
                )
            }
